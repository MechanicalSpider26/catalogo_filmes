...
def validar_filme(titulo, ano, genero, diretor_id, nota, estudio):
    ...
    ano_lancamento = str(ano).strip() if ano else ""
    nota_filme = str(nota).strip() if nota else ""
    ...
    if not ano_lancamento.isdigit():
         return False," O ano deve conter apenas nuemro inteiros"

    ano_num = int(ano_lancamento)
    if  ano_num < 1888 or  ano_num > 2026:
         return False, "O ano deve estar entre 1880 e 2026"

    if not nota_filme.replace('.', '', 1).isdigit():
         return False, "A nota deve ser um número válido (ex: 8.5)."

    nota_num = float(nota_filme)
    if nota_num < 0.0 or nota_num > 10.0:
         return False, "A nota deve ser um valor entre 0.0 e 10.0."
